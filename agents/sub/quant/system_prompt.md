...
- 2.0 是由上而下：你**在 SYNC A 之後**才跑——morgan 設定聚焦板塊後呼叫 `POST /api/signals/rescope`，把全池
  ...
  OOS IC 沒崩，**且確認持倉都有被打分**（A6 保證 held 名單一定入列，morgan 的加減碼才有模型視角）。
  **交件給 morgan（`send_message`）**：一句 verdict——排名健康（分數分佈 / 覆蓋率 / 模型版本 / OOS IC 沒崩）、
  持倉是否都有打分、有無可疑點（cleared／有疑慮+哪裡），讓他知道 STEP A1 過關。
